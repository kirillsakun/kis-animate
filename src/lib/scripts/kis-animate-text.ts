// @ts-ignore
import { InterfaceKisAnimateTextConstructorData } from './types.ts';
// @ts-ignore
import { TEXT_DEFAULTS, STATES } from './constants.ts';
// @ts-ignore
import splitBySymbols from '../helpers/split-by-symbols.ts';
		...
		const observerCallback: IntersectionObserverCallback = (entries: IntersectionObserverEntry[], observer: IntersectionObserver) => {
			for (let entryIndex = 0; entryIndex < entries.length; entryIndex += 1) {
				...
				if (entry.isIntersecting) {
					const { target } = entry;
					this.animateItem(target as HTMLElement, symbolsElements);
